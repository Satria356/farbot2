...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./worker/lib/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./worker/lib/myfunc')
...
// Setting
	...
	sock.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       if (options.asSticker || /webp/.test(mime)) {
        let { writeExif } = require('./worker/lib/exif')
    ...
    sock.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
        ...
        if (options.asDocument) type = 'document'
        if (options.asSticker || /webp/.test(mime)) {
            let { writeExif } = require('./worker/lib/sticker.js')
            let media = { mimetype: mime, data }
